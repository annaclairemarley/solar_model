...
energy_output = function(Gtotal, Tmin, Tmax, 
                         Tav, Tday_length, Tref, 
                         B, g, c1 = -3.75*10^-3, 
                         c2 = 1.14*10^-3, c3 = 0.0175*10^-3){
  ...
  eff = 1 - B*(Tcell - Tref) + g*log(Gday)
  
  # calculate solar output
  solar_output = Gday*eff
...
output_test <- energy_output(Gtotal = solar_stack_mask,
...
# rename bands
names(output_test) <- c("jan", "feb", "mar", "apr", "may", "jun", 
                    "jul", "aug", "sep", "oct", "nov", "dec")
plot(output_test)
