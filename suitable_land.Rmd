...
map_suitability_fun <- function(suitable_raster, year) {
  ...
  ### Calculating county rankings (scores) ###
...
```{r}
# Final score function
final_score_fun <- function (score_gen,
                             score_econ,
                             score_land,
                             weights = c(1, 1, 1)) {
  ...
  for (i in 1:nrow(scores_df)) {
    scores_df$final_score[i] <-
      weights[1] * scores_df$score_gen[i] + weights[2] * scores_df$score_land[i] + weights[3] * scores_df$score_econ[i]
  }
...
# Test function
final_score_df <-
  final_score_fun(score_gen, score_econ, county_maps_2020[[4]])
...
# Masking energy generation to suitability
```{r}
# Read files
...
# Resample
s_2020_resampled <- resample(s_2020, suitable_2020)

# Overlay
s_2020_overlay <- raster::overlay(s_2020_resampled,
                          suitable_2020,
                          fun = function (a, b){a*b})
# Convert to dataframe
s_2020_overlay_df <- raster::as.data.frame(s_2020_overlay, xy = TRUE)

# Replace 0s with NA
s_2020_overlay_df[s_2020_overlay_df == 0] <- NA

# Map
masked_2020_map <- ggplot() +
  geom_sf(data = CA_proj,
          fill = "gray90",
          color = "gray30", 
          size = 0.1) +
  geom_raster(data = s_2020_overlay_df,
               mapping = aes(x = x, y = y,  fill = layer))  +
  scale_fill_paletteer_c("ggthemes::Temperature Diverging",
                         name = "Energy Output (W/m^2)",
  ...
  geom_sf(data = counties_proj,
   ...
   theme_minimal() +
  coord_sf(datum=NA) +
  labs(x = "", y = "", title = "Solar Energy in CA (2020)") +
  annotation_scale(location = "bl", style = "ticks") # scale bar
